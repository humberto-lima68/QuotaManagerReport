		...
		onTheMasterPage: {
			...
			actions: {
				...
				iSortTheListOnName: function () {
					return this.iChooseASorter("sortButton", "Sort By <Znomesubst>");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList: function () {
					return this.waitFor({
						...
						success: function (oList) {
							var aEntityData = this.getEntitySet("zapo_acessoSet");
							var aObjectsNotInTheList = aEntityData.filter(function (oObject) {
								return !oList.getItems().some(function (oListItem) {
									return oListItem.getBindingContext().getProperty("Zusergc") === oObject.Zusergc;
								});
							});
							var sObjectNotInTheListId;

							if (aObjectsNotInTheList.length) {
								sObjectNotInTheListId = aObjectsNotInTheList[0].Zusergc;
							} else {
								// Not enough items all of them are displayed so we take the last one
								sObjectNotInTheListId = aEntityData[aEntityData.length - 1].Zusergc;
							}
							...
							oCurrentItem.bindingPath = "/" + oList.getModel().createKey("zapo_acessoSet", {
								Zusergc: sObjectNotInTheListId
							});
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Zusergc"),
						title: oBindingContext.getProperty("Znomesubst")
					};
				}
			},
			...
			assertions: {
				...
				theListShouldBeSortedAscendingOnName: function () {
					return this.theListShouldBeSortedAscendingOnField("Znomesubst");
				},
				...
				theListShouldHaveAllEntries: function () {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function (oList) {
							var aAllEntities = this.getEntitySet("zapo_acessoSet");
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							return this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "text",
									value: "<zapo_acessoSet> (" + iExpectedLength + ")"
								}),
								success: function () {
